...
const ProductList = () => {
    ...
    const fetchProducts = async () => {
      try {
        const endpoint = categoryName
          ? `https://parth3105.pythonanywhere.com/api/products/category/${categoryName}`
  ...
  const handleSubmit = async (productId, current_BIDDER) => {
    ...
    try {
      ...
      const response = await axios.post('https://parth3105.pythonanywhere.com/api/insertUserProduct/', {
        username: username,
        ProductId: productId,
      });
